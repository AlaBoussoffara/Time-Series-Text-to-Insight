...
class POSTGRES_EXEC_SQL(Action):
    ...
    def get_action_description(cls) -> str:
        return """
## POSTGRES_EXEC_SQL Action
* Signature: POSTGRES_EXEC_SQL(sql_query="SELECT * FROM your_table", is_save=True, save_path="/workspace/output_file.csv")
* Description: Executes a SQL query on POSTGRESQL. If `is_save` is True, the results are saved to a specified CSV file; otherwise, results are printed.
...
* Examples:
  - Example1: POSTGRES_EXEC_SQL(sql_query="SELECT count(*) FROM sales", is_save=False)
  - Example2: POSTGRES_EXEC_SQL(sql_query="SELECT user_id, sum(purchases) FROM transactions GROUP BY user_id", is_save=True, save_path="/workspace/result.csv")
"""

    @classmethod
    def parse_action_from_text(cls, text: str) -> Optional['POSTGRES_EXEC_SQL']:
        pattern = r'POSTGRES_EXEC_SQL\(sql_query=(?P<quote>\"\"\"|\"|\'|\"\"|\'\')(.*?)(?P=quote), is_save=(True|False)(, save_path=(?P<quote2>\"|\'|\"\"|\'\')(.*?)(?P=quote2))?\)'
    ...
    def __repr__(self) -> str:
        ...
        return f'POSTGRES_EXEC_SQL(sql_query="{self.sql_query}", is_save={self.is_save}{save_info})'
    
    
@dataclass
class POSTGRES_GET_TABLES(Action):
...
* Description: Executes a query to fetch all table names and their corresponding DDL from the specified dataset in POSTGRESQL. The results are saved to the specified CSV file.
  - The POSTGRES id of a table is usually in the form of database_name.dataset_name.table_name. This action mainly focuses on the tables under dataset_name.
* Examples:
  - Example1: GET_TABLES(database_name="POSTGRES-public-data", dataset_name="new_york", save_path="dataset_metadata.csv")
"""
    @classmethod
    def parse_action_from_text(cls, text: str) -> Optional[Action]:
...
@dataclass
class POSTGRES_GET_TABLE_INFO(Action):
...
* Description: Executes a query to fetch all column information (field path, data type, and description) from the specified table in the dataset in POSTGRESQL. The results are saved to the specified CSV file.
 - The POSTGRES id of a table is usually in the form of database_name.dataset_name.table_name.
* Examples:
  - Example1: GET_TABLE_INFO(database_name="postgres", dataset_name="samples", table="shakespeare", save_path="shakespeare_info.csv")
"""
    @classmethod
    def parse_action_from_text(cls, text: str) -> Optional[Action]:
...
@dataclass
class POSTGRES_SAMPLE_ROWS(Action):

    action_type: str = field(default="postgres_sample_rows",init=False,repr=False,metadata={"help": 'type of action, c.f., "postgres_sample_rows"'})
    ...
    def get_action_description(cls) -> str:
        return """
## POSTGRES_SAMPLE_ROWS Action
* Signature: POSTGRES_SAMPLE_ROWS(database_name="your_database_name", dataset_name="your_dataset_name", table="table_name", row_number=3, save_path="path/to/output_file.json")
* Description: Executes a query to sample a specified number of rows from the table in the dataset in POSTGRESQL using the TABLESAMPLE SYSTEM method. The results are saved in JSON format to the specified path.
* Examples:
  - Example1: POSTGRES_SAMPLE_ROWS(database_name="postgres", dataset_name="samples", table="shakespeare", row_number=3, save_path="shakespeare_sample_data.json")
"""
    @classmethod
    def parse_action_from_text(cls, text: str) -> Optional[Action]:
        matches = re.findall(r'POSTGRES_SAMPLE_ROWS\(database_name=(.*?), dataset_name=(.*?), table=(.*?), row_number=(.*?), save_path=(.*?)\)', text, flags=re.DOTALL)
        if matches:
