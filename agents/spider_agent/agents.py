...
class PromptAgent:
    ...
    def set_env_and_task(self, env: Spider_Agent_Env):
        ...
        if self.env.task_config['type'] == 'Postgres':
            self._AVAILABLE_ACTION_CLASSES = [Bash, Terminate, POSTGRES_EXEC_SQL, CreateFile, EditFile]
            action_space = "".join([action_cls.get_action_description() for action_cls in self._AVAILABLE_ACTION_CLASSES])
            self.system_message = POSTGRES_SYSTEM.format(work_dir=self.work_dir, action_space=action_space, task=self.instruction, max_steps=self.max_steps)
